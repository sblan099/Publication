...
```
R code to determine the distance at which each land class (forest/vegetation, urbanization, open water, wetland, and agriculture) had the maximum effect on the turtles behaviours (active defensive behaviours (Aggression), time of shell emergence (Shell), binary shell emergence time (Bin.Shell), time of initial movement (Start), log (x+1) transformed version of time of initial movement (log.Start), and total time spent moving (Movement)). Shell and Start were transformed in "Mixed Models Behaviour Code" so I will need to recalculate the distance at which each land cover class has a maximum impact on the transformed behaviours (Bin.Shell and log.Start). 
