...
## Plot data
We are doing this step to make sure we use the right data distribution for our models later on. If we do need to transform any of the data distributions, we will need to recalculate the distance at which each land cover class has a maximum impact on the transformed behaviour.
```{r message=FALSE}
...
Time of initial movement has a skewed distribution so we will plot a log transformed version of it. 
...
The log transformed version of time of initial movement is still somewhat skewed, so we will run our analysis on both versions for comparison. Shell emergence time is also skewed, so we will plot the binary version of it.
...
We need to create a new dataset that excludes all turtles with no observations for handling reactions
```{r}
...
```{r}
#We deleted the rows in Microsoft Excel
...
#### Creation of the dataset
We need to create a new dataset that excludes all turtles with no observation for shell emergence time (binary)
```{r}
...
#### Creation of the dataset
We need to create a new dataset that excludes all turtles with no observation for time of initial movement
```{r}
...
#### Creation of the dataset
We need to create a new dataset that excludes all turtles with no observation for the log(x+1) transformed version of time of initial movement
```{r}
...
#### Creation of the dataset
We need to create a new dataset that excludes all turtles with no observation for total time spent moving
```{r}
